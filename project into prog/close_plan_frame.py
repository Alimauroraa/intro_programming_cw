...
def close_plan_frame(parent):
    ...
    # label
    tk.Label(close_plan_frame, text="Plan ID", font="calibri 12", bg="#021631", fg="#fff").place(x=260,y=200)
    ...
    plan_id_entry = ttk.Combobox(close_plan_frame, values=[i for i in map(str, plan_df['PlanID'].tolist())], width=10)
    end_entry = tk.Entry(close_plan_frame, width=29, bd=2, font="calibri 10")

    plan_id_entry.place(x=245, y=240)
    end_entry.place(x=190, y=340)

    # button
    tk.Button(close_plan_frame, text="Back", bg="#FFFFFF", fg="black", width=10, height=1,
              command=lambda: back(close_plan_frame)).place(x=170, y=550)
    tk.Button(close_plan_frame, text="Clear", bg="#FFFFFF", fg="black", width=10, height=1, command=clear).place(x=270,
                                                                                                                  y=550)
    tk.Button(close_plan_frame, text="Submit", bg="#FFFFFF", fg="black", width=10, height=1,
              command=submit_date).place(x=370, y=550)
